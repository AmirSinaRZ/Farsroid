...
public class SearchActivity extends AppCompatActivity {
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		_api_request_listener = new RequestNetwork.RequestListener() {
			...
			public void onResponse(String _param1, String _param2, HashMap<String, Object> _param3) {
				...
				org.jsoup.select.Elements el3 = doc_1.select("img");
				for (int i = 0; i < (int)(el1.size()); i++) {
					...
					map.put("farsroid_img", el3.get(i * 4).attr("data-src"));
					searchListMap.add(map);
				}
				_refreshRecycler();
			}
	...
	private void initializeLogic() {
		_ui();
		_search(getIntent().getStringExtra("searchKey"));
	}
	
	public void _ui() {
		topText.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/vazir.ttf"), 0);
		toolbar.setElevation((float)5);
		topText.setText("نتایج جستجو برای \"".concat(getIntent().getStringExtra("searchKey").concat("\"")));
	...
	public void _search(final String _keyword) {
		api.startRequestNetwork(RequestNetworkController.GET, Preferences.main_url.concat("?s=".concat(_keyword)), "SearchActivity", _api_request_listener);
	}
	...
	public void _setupPages(final String _Html) {
		...
		for (int i = 0; i < (int)(el1.size() / 2); i++) {
			{
	...
	public void _refreshRecycler() {
		...
		searchRecycler.setLayoutManager(new GridLayoutManager(this, 3));
	}
	
	
	public void _searchPage(final String _page) {
		api.startRequestNetwork(RequestNetworkController.GET, Preferences.main_url.concat("page/".concat(_page.concat("/?s=".concat(getIntent().getStringExtra("searchKey"))))), "SearchActivity", _api_request_listener);
	}
